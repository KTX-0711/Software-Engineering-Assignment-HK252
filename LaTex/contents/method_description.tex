...
\begin{itemize}
    ...
    \begin{itemize}
        \item Mục đích: Kích hoạt luồng xem lịch sử giao dịch cá nhân của khách hàng có tài khoản.
    ...
    \begin{itemize}     
        \item Mục đích: Xử lý thao tác thanh toán một hóa đơn điện tử thông qua cổng thanh toán.

        \item Mô tả logic: Phương thức này tiếp nhận lệnh ``Thanh toán'' từ người dùng trên một hóa đơn cụ thể và gọi đến API của cổng thanh toán BKPay. Nó chờ Webhook trả về và xử lý kết quả.      
    \end{itemize}

    \item \textbf{generateInvoice(): void (Class Invoice)}
    \begin{itemize}
        \item Mục đích: Khởi tạo thông tin chi tiết cho một hóa đơn mới.
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            \item Trong U3.2: Được gọi sau khi \texttt{payInvoice()} nhận kết quả trả về từ BKPay để đổi trạng thái từ ``Pending'' sang ``Success'' hoặc ``Failed''.
